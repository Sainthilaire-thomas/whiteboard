...
const Postit: React.FC<PostitProps> = memo(({ inline = false }) => {
  ...
  const isStepAccessible = (step: number) => {
    ...
    if (step === 3) return isCompleted; // La synthèse est accessible uniquement si complété
    return false;
  };
  ...
  const handleFinalizeStep = () => {
    if (activeStep === 0) {
      // Passer à l'étape suivante depuis le contexte
      handleNext();
    } else if (activeStep === 1) {
      // Vérifier que le sujet est sélectionné avant de continuer
      if (selectedPostit.idsujet) {
        handleNext();
      }
    } else if (activeStep === 2) {
      // Finaliser et enregistrer à l'étape 3
      if (
        selectedPostit.pratique &&
        selectedPostit.pratique !== "Non Assigné"
      ) {
        handleSave();
        setActiveStep(3); // Passer à la synthèse après sauvegarde
      }
    } else if (activeStep === 3) {
      // Terminer à l'étape de synthèse
      handleClosePostit();
    }
  };

  // Déterminer l'étiquette du bouton de finalisation
  const getFinalizeButtonLabel = () => {
    if (activeStep === 0 || activeStep === 1) return "Continuer";
    if (activeStep === 2) return "Finaliser";
    if (activeStep === 3) return "Terminer";
    return "Suivant";
  };

  // Vérifier si le bouton de finalisation doit être désactivé
  const isFinalizeButtonDisabled = () => {
    if (activeStep === 1 && !selectedPostit.idsujet) return true;
    if (
      activeStep === 2 &&
  ...
  const steps = [
    {
      ...
      content: (
        <ContexteStep
          ...
          styles={styles}
          // Supprimer onNext car la navigation sera gérée par le stepper
        />
      ),
      ...
      <MobileStepper
        ...
        nextButton={
          <Button
            ...
            onClick={handleNext}
            disabled={
              activeStep === 2 || (activeStep === 1 && !selectedPostit.idsujet)
            }
            sx={{
              bgcolor:
  ...
  // Interface de navigation horizontale pour les étapes (apparaît au-dessus du contenu)
      ...
      >
        {steps.map((step, index) => (
          <Step key={index} completed={step.completed}>
            ...
            >
              {/* Simplifier le contenu à juste le texte */}
              <Typography
                variant="body2"
                sx={{
              ...
              {/* Conserver les informations additionnelles si nécessaires */}
              {step.additionalInfo && (
                <Typography
                  component="span"
                  variant="caption"
    ...
    <>
      ...
      >
        ...
        <Box sx={{ display: "flex", alignItems: "center" }}>
          ...
          {/* Bouton de fermeture compact toujours visible */}
          <Button
      ...
      <DialogContent sx={{ display: "flex", flexDirection: "column" }}>
        ...
        >
          {/* Partie gauche : actions secondaires (toujours présentes) */}
          <Box>
            <Button
              variant="outlined"
              onClick={handleBack}
          ...
          {/* Partie droite : action principale (contextuelle) */}
          <Box>
            {activeStep < 3 && (
              <Button
                variant="contained"
  ...
  console.log("Affichage SummaryPanel:", isCompleted === true);
  console.log("selectedPostit:", selectedPostit);
  console.log("theme disponible:", !!theme);
  console.log("stepBoxStyle disponible:", !!stepBoxStyle);
  console.log("handleClosePostit disponible:", !!handleClosePostit);
